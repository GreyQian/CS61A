...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    step, score, sow_sad = 1, 0, False
    while step <= num_rolls:
        roll = dice()
        if roll == 1:
            sow_sad = True
        step += 1
        score += roll
    return score if not sow_sad else 1
    # END PROBLEM 1
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def roll(current_score, opponent_score):
        return n
    return roll
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    dice_num_list = [0 for i in range(0, 11)]
    for i in range(0, goal):
        for j in range(0, goal):
            dice_num = strategy(i, j)
            dice_num_list[dice_num] = 1
            roll_dice_num = sum(dice_num_list)
            if roll_dice_num > 1:
                return False
    return True
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    index, max_score = 0, 0 
    for i in range(1, 11):
        score = make_averaged(roll_dice, total_samples)(i, dice)
        if score > max_score:
            index, max_score = i, score
    return index
    # END PROBLEM 9
