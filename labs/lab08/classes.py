...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        if implemented:
            ...
            cards = [player.deck.draw() for _ in range(2)]
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        """
        "*** YOUR CODE HERE ***"
        best_card, best_power = None, -float("inf")
        for card in player.hand:
            if card.power(opponent_card) > best_power:
                best_card, best_power = card, card.power(opponent_card)
        # You should add your implementation above this.
        if best_card:
